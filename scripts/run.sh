...
# This script starts N nodes (TODO N instead of 5) and waits for ctrl-c to shutdown the process group of MetalGo processes
# Uses data directory to store all MetalGo data neatly in one location with minimal config overhead
...
CMD="$METALGO_PATH --data-dir=$NODE_DATA_DIR --config-file=$NODE_CONFIG_FILE_PATH"
